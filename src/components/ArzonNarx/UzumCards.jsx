  ...
  return (
    <section className="w-full lg:flex-1">
          ...
          return (
            ...
            >
              ...
              <div className="p-3 flex-1 flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1 text-purple-600 font-bold text-3xl">
                  {product.price} so'm
                </div>

                <div className="min-h-[20px] text-gray-400 text-2xl">
                  <span className="line-through">{product.oldPrice} so'm</span>
                </div>

                <div className="w-fit bg-yellow-300 text-black text-sm px-2 py-1 rounded-lg">
                  {product.monthly} so'm/oyiga
                </div>

                <p className="text-xl text-gray-800 line-clamp-2 min-h-[64px]">
                  {product.title}
                </p>
                ...
                <button
                  ...
                  className="mt-auto w-full bg-purple-600 hover:bg-purple-700 text-white py-3 rounded-xl text-xl font-medium"
                >
                  Savatga qo'shish
                </button>
              </div>
      ...
      {selectedProduct && (
        ...
        >
          <div
            className="w-full max-w-5xl rounded-2xl overflow-hidden bg-[#ececef] grid lg:grid-cols-[420px_1fr]"
          ...
          >
            ...
            <div className="p-5 md:p-8">
              <div className="flex items-start justify-between gap-4">
                <h2 className="text-2xl md:text-4xl font-bold text-[#1a2233] leading-tight">
                  {selectedProduct.title}
                </h2>
                <button
                  onClick={closeProductModal}
                  className="w-10 h-10 rounded-full bg-[#d4d6dd] text-2xl text-[#626874]"
                  aria-label="Yopish"
                >
                  x
                </button>
              ...
              <p className="mt-4 text-4xl md:text-5xl font-extrabold text-violet-600">
                {Number(selectedProduct.price.replace(/\D/g, "")).toLocaleString("ru-RU")} so'm
              </p>
              <p className="mt-2 text-[#667084] text-lg md:text-xl">
              ...
              <div className="mt-8 flex items-center gap-4">
                <button
                  ...
                  className="w-16 h-16 rounded-2xl bg-[#d0d3db] text-4xl font-semibold text-[#2a2f3a]"
                >
                  -
                </button>
                <span className="w-14 text-center text-3xl font-semibold">{count}</span>
                <button
                  onClick={() => setCount((prev) => clampQuantity(prev + 1))}
                  disabled={count >= MAX_QTY}
                  className="w-16 h-16 rounded-2xl bg-violet-600 text-white text-4xl font-semibold disabled:opacity-60"
                >
                  +
                </button>
              </div>
              ...
              <button
                ...
                className="mt-8 w-full bg-violet-600 hover:bg-violet-700 text-white py-4 rounded-2xl text-2xl md:text-3xl font-semibold"
              >
                Savatga qo'shish
              </button>
            </div>
